...
namespace SPGenerator.Core
{
    public abstract class BaseSPGenerator
    {
        ...
        internal List<DBTableColumnInfo> _whereConditionFields;
        ...
        #region GenerateSP
        //Brent  this is where the sp is assembled
        ...
        {
            _whereConditionFields = whereConditionFields;
        ...
        protected virtual void GenerateInputParameters(List<DBTableColumnInfo> tableFields, StringBuilder sb)
        {
            foreach (DBTableColumnInfo colInf in tableFields)
            {
                if (colInf.Exclude)
                    continue;
                sb.Append(Environment.NewLine);
                sb.Append( "\t" + prefixInputParameter + colInf.ColumnName);
